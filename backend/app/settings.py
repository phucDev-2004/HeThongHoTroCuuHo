...
if ENV == 'production':
    ...
    ALLOWED_HOSTS = os.getenv('ALLOWED_HOSTS', 'cuuho.vpone.site,www.cuuho.vpone.site,localhost,127.0.0.1').split(',')
    
    # BẮT BUỘC cho Django 4.x khi chạy https (Production)
...
STATIC_URL = 'static/'
# Thư mục chứa static files khi chạy collectstatic (quan trọng cho Nginx phục vụ)
...
# ==================== CHANNELS (WebSocket) ====================
# Lưu ý: InMemoryChannelLayer không ổn định cho Production nhiều worker.
# Nếu sau này có điều kiện, hãy cài Redis và dùng RedisChannelLayer.
CHANNEL_LAYERS = {
    "default": {
...
# ==================== CORS Configuration ====================
if ENV == 'production':
    # Cho phép frontend tại domain này gọi API
    CORS_ALLOWED_ORIGINS = os.getenv('CORS_ALLOWED_ORIGINS', 'https://cuuho.vpone.site,https://www.cuuho.vpone.site').split(',')
...
if USE_CLOUD:
    ...
    # Cấu hình Local (Khi chạy trên máy tính hoặc VPS lưu file tại chỗ)
    MEDIA_URL = '/media/'
...
# ==================== WINDOWS GDAL CONFIG ====================
# Chỉ chạy đoạn này nếu đang ở trên Windows (Môi trường dev của bạn)
if os.name == 'nt':
    OSGEO_PATH = r"C:\Users\VanPhuc\AppData\Local\Programs\Python\Python39\Lib\site-packages\osgeo"
    
    # Kiểm tra xem đường dẫn có tồn tại không để tránh lỗi
